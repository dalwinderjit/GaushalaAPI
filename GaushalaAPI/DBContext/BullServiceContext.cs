...
{
    public class BullServiceContext 
    {
        private readonly IConfiguration _configuration;
        public BullServiceContext(IConfiguration configuration)
        {
        ...
        public Dictionary<string, object> GetServiceDetailByBullId(long id)
        {
            ...
            if (id != null)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand sqlcmd = new SqlCommand("Select * from CowConceiveData where BullID = @BullID ", conn);
                    try
                    {
                        sqlcmd.Parameters.Add("@BullID", System.Data.SqlDbType.BigInt);
                        sqlcmd.Parameters["@BullID"].Value = id;
                        ...
                        while (sqlrdr.Read())
                        {
                            ...
                            if(sqlrdr["BullID"].ToString().Trim()!=""){
                                Dictionary<string, string> bull_data  = BullsContext.GetBullNameTagNoById(_configuration, Convert.ToInt64(sqlrdr["BullID"]));
                                temp["bullsName"] = bull_data["tagNo"]+"/"+bull_data["name"];
                            }
                        ...
                        data["recordsTotal"] = this.GetTotalServiceDetailByBullId(id);
                        data["recordsFiltered"] = this.GetTotalServiceDetailByBullId(id);
                    }
                    catch (Exception ex)
                    {
                        //return false;
        ...
        public long GetTotalServiceDetailByBullId(long? id)
        {
            long counter=0;
            if (id != null)
            {
                ...
                {
                    //Console.WriteLine("Select count(*) as total from Animals where TagNo = @TagNo");
                    SqlCommand sqlcmd = new SqlCommand("Select count(*) as total from CowConceiveData where BullID = @BullID", conn);
                    try
                    {
                        sqlcmd.Parameters.Add("@BullID", System.Data.SqlDbType.BigInt);
                        sqlcmd.Parameters["@BullID"].Value = id;
        ...
        internal Dictionary<string, object> AddBullServiceData(CowServiceDataModel conceive)
        {
            ...
            if (conceive.ValidateAdd() == true)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        if (conceive.PregnancyStatus == "4" || conceive.PregnancyStatus == "Successful")
                        {
                            if ((conceive.DeliveryStatus == "1" || conceive.DeliveryStatus == "2"))
                            {
                                ...
                                if (message == "" || pregnancy_status == false)
                                {
                                    ...
                                    data2["message"] = "Bull Service Data Added Failed, Unable to add Calv";
                                ...
                                else
                                {
                                    ...
                                    if(conceive.animal.SaveImage2()==true){
                                        tran.Commit();
                                        Dictionary<string, string> data2 = new Dictionary<string, string>();
                                        data2["message"] = $"Bull Service {i} Data Added Successfully" + message;
                                        ...
                                        Dictionary<string, string> data2 = new Dictionary<string, string>();
                                        data2["message"] = $"Bull Service Data Addition Failed Image error" + message;
                                        data2["status"] = "failure";
                                        data["data"] = data2;
                                    }
                                }
                            ...
                            else if (conceive.DeliveryStatus == "3") //CHild died while birth
                            {
                                ...
                                if (pregnancy_status == true && i > 1) { 
                                    tran.Commit();
                                    Dictionary<string, string> data2 = new Dictionary<string, string>();
                                    data2["message"] = $"Bull Service {i} Data Added Successfully.Child Died";
                                    data2["status"] = "success";
                                    data["PregnancyStatus"] = pregnancy_status_;
                                ...
                                {
                                    Dictionary<string, string> data2 = new Dictionary<string, string>();
                                    data2["message"] = $"Bull Service {i} Data Updation Failed.";
                                    data2["status"] = "failure";
                                    data["data"] = data2;
                                }
                            }
                            ...
                            {
                                Dictionary<string, string> data2 = new Dictionary<string, string>();
                                data2["message"] = "Bull Service Data Addition Failed. Invalid Delivery Status";
                                data2["status"] = "failure";
                                data["data"] = data2;
                            }
                        }
                        else
                        {
                            ...
                            if (i > 0 && pregnancy_status == true)
                            {
                                tran.Commit();
                                Dictionary<string, string> data2 = new Dictionary<string, string>();
                                data2["message"] = "Bull Service Data Added Successfully";
                                data2["status"] = "success";
                                data["data"] = data2;
                                data["PregnancyStatus"] = pregnancy_status_;
                            }
                            else
                            {
                                tran.Rollback();
                                Dictionary<string, string> data2 = new Dictionary<string, string>();
                                data2["message"] = "Bull Service Data Addition Failed";
                                data2["status"] = "failure";
                                data["data"] = data2;
                            }
                        }
                    }catch(Exception e)
                    {
                        ...
                        Dictionary<string, string> data2 = new Dictionary<string, string>();
                        data2["message"] = "Bull Service Data Addition Failed"+e.Message;
                        data2["status"] = "failure";
                        data["data"] = data2;
                    }
                }
        ...
        internal Dictionary<string, object> UpdateBullServiceData(CowServiceDataModel conceive)
        {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    if (conceive.PregnancyStatus == "4" || conceive.PregnancyStatus == "Successful")
                    {
                        if (conceive.DeliveryStatus == "1" || conceive.DeliveryStatus == "2")
                        {
                            ...
                            if (calv_Data != null && lac_data != null)
                            {
                                ...
                                if (status == "success" && AnimalID!=0 && message!="") {
                                    try
                                    {
                                        ...
                                        if (i > 0 && pregnancy_status==true && newLactaionNumber!=0)
                                        {
                                            ...
                                            conceive.animal.SaveImage2();
                                            Dictionary<string, string> data2 = new Dictionary<string, string>();
                                            data2["message"] = "Bull Service Data Updated Successfully. "+message;
                                            data2["status"] = "success";
                                            data["PregnancyStatus"] = pregnancy_status_;
                                        ...
                                        else
                                        {
                                            ...
                                            tran.Rollback();
                                            Dictionary<string, string> data2 = new Dictionary<string, string>();
                                            data2["message"] = "Bull Updation Failed";
                                            data2["status"] = "failure";
                                            data["data"] = data2;
                                        }
                                    }
                                    catch (Exception e)
                                    {
                                        ...
                                        tran.Rollback();
                                        Dictionary<string, string> data2 = new Dictionary<string, string>();
                                        data2["message"] = "Bull Updation Failed. "+e.Message;
                                        data2["status"] = "failure";
                                        data["data"] = data2;
                                    }
                                }
                                ...
                                {
                                    Dictionary<string, string> data2 = new Dictionary<string, string>();
                                    data2["message"] = "Bull Service Data UPdation Failed . " + message;
                                    data2["status"] = "failure";
                                    data["data"] = data2;
                                }
                                return data;
                    ...
                    try
                    {
                        ...
                        if (i > 0 && pregnancy_status ==true)
                        {
                            tran.Commit();
                            Dictionary<string, string> data2 = new Dictionary<string, string>();
                            data2["message"] = "Bull Service Data Updated Successfully";
                            data2["status"] = "success";
                            data["PregnancyStatus"] = pregnancy_status_;
                            data["data"] = data2;
                        }
            ...
            if (id != null)
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        {
                            CowsContext cowsContext = new CowsContext(_configuration);
                            Dictionary<long, object> cowsIdName = cowsContext.GetCowTagNoNameByIds(cow_ids);
                            try
                            {
                                Dictionary<string, string> tempData = (Dictionary<string, string>)cowsIdName[Convert.ToInt64(temp["cowID"])];
                                temp["cowName"] = tempData["name"];
                                temp["cowTagNo"] = tempData["tagNo"];
                            }catch(Exception e)
                            {
                                temp["cowName"] = "";
                                temp["cowTagNo"] = "";
